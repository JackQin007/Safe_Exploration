...
path = 'safety-gymnasium-main/safety_gymnasium/tasks/safe_isaac_gym/envs/cfgs/freight_franka_close_drawer.yaml'
...
def main():
    import argparse
    from safety_gymnasium.tasks.safe_isaac_gym.utils.process_sarl import process_ppo
    import safety_gymnasium.tasks.safe_isaac_gym.utils.config as gymcfg
    env = FreightFrankaCloseDrawer( cfg = data)
    log_directory = './log'
    args = gymcfg.get_args()
    path = 'safety-gymnasium-main/safety_gymnasium/tasks/safe_isaac_gym/envs/cfgs/freight_franka_close_drawer.yaml'
    with open(path, 'r') as yaml_file:
        data = yaml.load(yaml_file, Loader=yaml.FullLoader)
    train_config = data['trainning']
    ppo = process_ppo(args, env, train_config, log_directory)

if __name__ == '__main__': 
    env = FreightFrankaCloseDrawer( cfg = data)
    obs, rew,cost,reset_buf,info = env.reset()  # pylint: disable=unused-variable
    ...
    while True:
        ...
        act = env.actions
        # 创建一个与原张量相同大小的随机数张量，范围在 [0, 1) 之间
        random_tensor = torch.rand(act.shape, device=act.device)
        # 定义随机化范围
        min_value = -1.0
        max_value = 1.0
        # 随机化操作
        randomized_tensor = random_tensor * (max_value - min_value) + min_value
        # 添加随机化后的值到原始张量上
        randomized_result = act + randomized_tensor

        # print('obs',env._get_base_observation())
        # print('act',act)
        # pylint: disable-next=unused-variable
        obs, reward, cost, done , _= env.step(randomized_result)
